...
CREATE TABLE IF NOT EXISTS `blacklist_history` (
  ...
  `apply_date` date NOT NULL COMMENT '적용일자',
...
CREATE TABLE IF NOT EXISTS `sso_user` (
  ...
  `mod_datetime` datetime DEFAULT NULL COMMENT '수정일',
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `user_sort` char(2) NOT NULL DEFAULT 'NR' COMMENT '회원구분(회원분류가맹점과 일반을 구분하는 키값)',
  `last_login_date` datetime DEFAULT NULL COMMENT '최근 로그인 일자',
  `last_password_mod_date` datetime DEFAULT NULL COMMENT '최근 비밀번호 수정 일자',
  `login_try_count` int(1) NOT NULL DEFAULT 0 COMMENT '로그인 시도 횟수',
  `user_status` char(2) NOT NULL DEFAULT 'NR' COMMENT '회원상태코드 (정상, 탈퇴, 휴면)',
...
CREATE TABLE IF NOT EXISTS `user_roles` (
  ...
  `reg_user_no` bigint(20) NOT NULL DEFAULT 0,
  `mod_user_no` bigint(20) DEFAULT 0,
